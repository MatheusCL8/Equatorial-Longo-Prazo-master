...
def build_porcentagem(mes,data):
    ...
    dados=[]
    for linha in range(0,len(Data_t)-1):
        valores=[]
        for coluna in Data_t:
            #x = dataframe.loc[linha][coluna]
            x = ((Data_t.iloc[linha + 1][coluna] * 100) / Data_t.iloc[linha][coluna])-100
            valores.append('%.2f'%float(x))
        dados.append(valores)
        
    Date=Date.iloc[1:len(Date)]
    Date = Date.reset_index(drop=True)
    dataset=pd.DataFrame(dados,columns=Data_t.columns)
    dataset=pd.concat([Date,dataset],axis=1)
    return dataset
...
def Dynamic_Bayesian_Inference(topologia, top, ano,discretized_df,data,data_sup,mes,faixas,json_doc, ajuste, n_bins):
    ...
    for i in range(int(target[1])):
        ...
        if len(inferencia['ANO_'+str(i)])!=0 and 'MES_'+str(mes) in g_key and len(inferencia['ANO_'+str(i)]['MES_'+str(mes)])!=0:
            ...
            if(ajuste == 1):      
                if round(prob, 2) % round(1/n_bins, 2) == 0 and index_prob < (n_bins - 1):
                    index_prob=inf.index(prob)+1
                
            #print('Porcentagem: ', inf)
            ...
            print("Media",medflag)
            print('Max: ',maxflag,'\n')
            dict_local['ANO']=ano_atual
            dict_local['VALOR TOTAL']=maxflag
            ...
            nivel=['Baixo', 'Medio', 'Alto']
            '''if prob==0.5 and index_prob==0:
                nivel[index_prob]=nivel[index_prob+1]'''
            print(target[0],'--- Tempo'+'_'+str(i),'--- Probabilidade: ',prob,'--- Nivel: ',nivel[index_prob])
            print('Ano: ',ano_atual)
            print('Min: ',flag)
            print("Media",medflag)
            print('Max: ',maxflag,'\n')
            dict_local['ANO']=ano_atual#+(i+1)
            dict_local['VALOR TOTAL']=maxflag
            dict_local['PROBABILIDADE']=prob
            dict_local['NIVEL']=nivel[index_prob]
            dict_local['MEDIO']= medflag#(float(f[int(index_prob)])+float(f[int(index_prob)+1]))/2
...
def build_dbn(ano,topologia, top, nodes, dataframe, n_bins, tipo_discretizacao, json_doc, usuario, ajuste):
    meses=['JANEIRO','FEVEREIRO','MARCO','ABRIL','MAIO','JUNHO','JULHO','AGOSTO','SETEMBRO','OUTUBRO','NOVEMBRO','DEZEMBRO']
    dic_infer=dict()

    for m in range(len(meses)):
        mes=m
        dataset=build_porcentagem(mes, dataframe)
        ...
        infer=Dynamic_Bayesian_Inference(topologia, top,ano,discretized_data,data,data_sup,mes, faixas, json_doc, ajuste, n_bins)
